...
const CODEX_INSTRUCTIONS = `Use ${toolNames.read} for direct file reads, apply_patch for all project-file modifications, exec_command for inspection, tests, builds, package scripts, system diagnostics, database clients such as mysql, mysqlsh, psql, sqlite3, and sqlcmd, service CLIs such as redis-cli, docker, and kubectl, and other development commands, and write_stdin to poll or interact with running processes. Database and service commands may read or modify external system state when the user's task requires it. Do not use exec_command as a workaround for directly modifying project files; use apply_patch for project-file changes. Commands run with the local user's authority and are not sandboxed; workspace validation only selects their initial working directory. Follow instructions returned by ${toolNames.openWorkspace}; read applicable instruction and skill files before working in their scope.`;
...
function registerCodexProcessTools(context: ToolRegistrationContext): void {
  ...
  server.registerTool(
    "exec_command",
    {
      ...
      description:
        "Run a command with the local user's authority. Commands are not sandboxed; workspace validation only selects the initial working directory. Returns the result when it exits during the yield window, otherwise returns a sessionId for write_stdin. Use this for inspection, tests, builds, package scripts, system diagnostics, database clients such as mysql, mysqlsh, psql, sqlite3, and sqlcmd, service CLIs such as redis-cli, docker, and kubectl, and other development operations. Database and service commands may read or modify external system state when required by the user's task. Do not use exec_command as a workaround for directly modifying project files; use apply_patch for project-file changes.",
      inputSchema: {
        workspaceId: z.string().describe(workspaceIdDescription),
